...
class DashBoard extends Component {
  ...
  render() {
    ...
    return (
      <>
        {auth.uid ? (
          <SideBar expanded={isExpanded} collapse={this.handleCollapse} expand={this.handleExpand} users={users} />
        ) : (
          <SideBar expanded={false} collapse={this.handleLogin} expand={this.handleLogin} />
        )}
...
export default compose(
  ...
  firestoreConnect(props => [
    ...
    { collection: 'users', where: ['state', '==', 'online'] },
    { collection: 'chats', doc: 'mainChat', storeAs: 'mainChat' },
  ])
)(DashBoard)
