...
export default class ActionPower extends Component {
        ...
        return (
          <div key={essence.type} className={'icons '}>
            <div className={'essence sm ' + essence.type + classModifier}>
              ...
              {signModifier}
              {essence.quantity > 1 || essence.quantity < 0 ? essence.quantity : ''}
            </div>
            {plus && !isLast && <div className="operator mt-n2 mr-0 ml-0">+</div>}
            {slash && !isLast && (
              <div className="operator mt-n2">
  ...
  renderActionCost = action => {
    let essencePrefix =
      action.cost.essenceList && Object.keys(action.cost).filter(key => key.startsWith('turn')).length > 0
    ...
    return (
      <>
        ...
        {essencePrefix && <>+</>}
        {essences}
        {action.cost.onComponent && (
          <>
            <div className="action-text">on</div>
            <div className="icon on-component-icon"></div>
          </>
        )}
        {action.cost.sameType && (
          <>
            +{' '}
            <div className="essence sm any-same-type">
        ...
        {action.cost.destroyOneArtefact && (
          <>
            +
            <div className="action-text max-85">
              destroy <i>any one</i> of your artifacts
            </div>
          </>
        )}
  ...
  renderActionGain = action => {
    ...
    return (
      <>
        ...
        {essences}
        {rivalsGainPrefix && <div className="action-text">+ all rivals gain</div>}
        {rivalsGainEssences}
        {action.gain.rivalsLoseLife > 0 && (
          <>
            <div className="action-text">all rivals</div>
            <div className="icon essence sm life-loss">
              {action.gain.rivalsLoseLife > 1 ? action.gain.rivalsLoseLife : ''}
            </div>
          </>
        )}
        ...
        {action.gain.drawOne && (
          <div className="action-text">
            draw <span className="large">1</span> card
          </div>
        )}
        {action.gain.drawThreeDiscardThree && (
          <div className="action-text max-125">
            draw <span className="large">3</span> cards, add to hand, discard <span className="large">3</span>
          </div>
        )}
        {action.gain.reorderThree > 0 && (
          <div className="action-text">
            draw <span className="large">3</span> cards, reorder, put back <i>(may also use on Monument deck)</i>
          </div>
        )}
        {action.gain.onComponent && (
          <>
            <div className="action-text mr-1">on</div>
            <div className="icon on-component-icon"></div>
          </>
        )}
        {action.cost.onlyWhenTurned && <div className="action-text max-95">(When this card is turned)</div>}
        {action.gain.putItOnAnyComponent && (
          <>
            <div className="action-text max-60">put the essence spent on</div>
            <div className="icon component-icon"></div>
          </>
        )}
        {action.gain.putItOnComponent && (
          <>
            <div className="action-text">put it on</div>
            <div className="icon on-component-icon"></div>
          </>
        )}
        {action.gain.placeArtefactFromDiscard && (
          <>
            <div className="action-text max-75">
              place one of <i>your</i> discards at
            </div>
        ...
        {action.gain.powerCostAsGold && (
          <div className="icon essence sm gold">
        ...
        {action.gain.placementCostAsGold && (
          <>
            ...
            <div className="icon placement-cost-icon max-95">
              <div className="qm-dark"></div>
            </div>
        ...
        {action.gain.asManyCalmThanRivalsElan && (
          <>
            <div className="action-text">Gain</div>
            <div className="essence sm calm">
              <div className="qm"></div>
            </div>
            <div className="action-text">equal to</div>
            <div className="essence sm elan">
              <div className="qm"></div>
            </div>
        ...
        {action.gain.asManyElanThanRivalsDeath && (
          <>
            ...
            <div className="action-text">equal to</div>
            <div className="essence sm death">
              <div className="qm"></div>
            </div>
            <div className="action-text">of one rival</div>
          </>
        ...
        {action.gain.placeDragonFromAnyDiscardPile && (
          <>
            ...
            <div className="icon dragon-icon"></div>
            <div className="action-text max-95">
              from <i>any</i> player's discard pile at
            </div>
            <div className="icon placement-cost-icon">
              <div className="qm-dark"></div>
            </div>
  ...
  render() {
    let { component, action, disabled, index, onClick, selected } = this.props
    ...
    return (
      <div className={'component-action position-' + index + color + disable + select} onClick={onClick}>
