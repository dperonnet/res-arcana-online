  ...
  renderEssencePicker = () => {
    ...
    let picker = essenceList.map((type, index) => {
      ...
      return (
        <div
          key={type}
          className={'collect-option ' + fade + pointerLock}
          onClick={() => (isValid && !lock ? handleOnClick(type) : null)}
        >
          <div
            className={'essence ' + type + pointer}
            onClick={() => (!isValid && enabled && !lock ? handleOnClick(type) : null)}
          >
            {selection[type] || 0}
          </div>
          {!isLast && (
            <div className={'operator ' + hidePlus}>
    ...
    return <div className={isValid ? ' delete-cursor' : ' '}>{picker}</div>
  }

  render() {
    return (
      <div className="essence-picker">
        <div className={'collect-options'}>{this.renderEssencePicker()}</div>
      </div>
    )
  }
}
